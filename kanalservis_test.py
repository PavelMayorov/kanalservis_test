...
def get_usd_exchange_rate() -> float:
    ...
    try:
        ...
        logger.error(f'Нет соединения с сайтом ЦБ РФ!\n{ex}')
    else:
...
def get_data_spreadsheet() -> list:
    ...
    try:
        ...
        logger.error(f'Нет соединения с сайтом Google Sheets!\n{ex}')
    else:
...
def create_dataframe(values: list, rate: float):
    ...
    try:
        ...
        try:
            ...
            logger.error(f'Столбец "стоимость,$" заполнен не корректно!\n{ex}')
        else:
...
def save_to_db(df):
    ...
    try:
        ...
        logger.error(f'Не удалось подключится к базе данных!\n{ex}')
    else:
